...
exports.compareFileAcrossCommits = compareFileAcrossCommits;
...
function parseCommitMetadata(line) {
    const [hash, shortHash, date, author, authorEmail, ...messageParts] = line.split("|");
    return {
        hash,
        shortHash,
        date,
...
function parseCommitSummary(line) {
...
async function loadParsedHistory(git, filePath, maxCount) {
...
async function resolveFilePathAtCommit(git, filePath, commitHash) {
    const parsedHistory = await loadParsedHistory(git, filePath);
    const matchingEntry = parsedHistory.entries.find((entry) => entry.hash === commitHash);
    if (!matchingEntry) {
        return normalizeGitPath(filePath);
    }
    return matchingEntry.pathAtCommit;
}
async function getCommitInfo(git, commitHash) {
 ...
 */
async function getFileHistory(repoPath, filePath, maxCount) {
 ...
 */
async function compareFileAcrossCommits(repoPath, filePath, fromCommitHash, toCommitHash) {
    const git = (0, simple_git_1.default)(repoPath);
    const [fromCommit, toCommit, fromPathAtCommit, toPathAtCommit] = await Promise.all([
 ...
 */
async function getFileAtCommit(repoPath, filePath, commitHash) {
    const git = (0, simple_git_1.default)(repoPath);
 ...
 */
async function restoreFileFromCommit(repoPath, filePath, commitHash) {
 ...
 */
async function getFileStatistics(repoPath, filePath) {
    const git = (0, simple_git_1.default)(repoPath);
    ...
    const ageInDays = Math.max(0, Math.ceil((lastDate.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24)));
 ...
 */
async function getFileVersions(repoPath, filePath, commitHashes) {
    const versions = [];
 ...
 */
async function getLineHistory(repoPath, filePath, startLine, endLine) {
    const git = (0, simple_git_1.default)(repoPath);
