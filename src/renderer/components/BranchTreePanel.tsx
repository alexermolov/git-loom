...
const BranchTreePanel: React.FC<BranchTreePanelProps> = ({
  ...
  const handleCreateBranch = (fromCommit?: string) => {
    modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          await window.electronAPI.createBranch(
            repoPath,
            branchName,
            fromCommit,
            switchAfterCreate,
      ...
      onOk: async () => {
        try {
          if (isRemote) {
            ...
            message.success(
              `Remote branch "${displayName}" deleted successfully`,
            );
          } else {
            ...
            if (result.success) {
              ...
              modal.confirm({
                ...
                onOk: async () => {
                  try {
                    await window.electronAPI.deleteBranch(
                      repoPath,
                      branchName,
                      true,
                    );
                    message.success(
                      `Branch "${branchName}" force deleted successfully`,
                    );
                    onRefresh?.();
                  } catch (error: any) {
  ...
  const handleRenameBranch = (branchName: string) => {
    modal.confirm({
      ...
      onOk: async () => {
        const input = document.getElementById(
          "new-branch-name-rename",
        ) as HTMLInputElement;
        const checkbox = document.getElementById(
          "rename-remote-checkbox",
        ) as HTMLInputElement;
        ...
        try {
          ...
          message.success(
            `Branch renamed from "${branchName}" to "${newName}"`,
          );
          onRefresh?.();
        } catch (error: any) {
  ...
  const handleSetUpstream = (
    ...
    const remoteOptions = Array.from(
      new Set(
        remoteBranches.map((b) => b.name.replace("remotes/", "").split("/")[0]),
      ),
    );
    ...
    modal.confirm({
      ...
      cancelText: "Cancel",
      onOk: async () => {
        const remoteSelect = document.getElementById(
          "upstream-remote-select",
        ) as any;
        const branchInput = document.getElementById(
          "upstream-branch-input",
        ) as HTMLInputElement;
        ...
        try {
          await window.electronAPI.setUpstreamBranch(
            repoPath,
            branchName,
            remote,
            remoteBranchName,
  ...
  const handleCompareBranches = (branchName: string) => {
    Modal.info({
      ...
      onOk: async () => {
        try {
          const comparison = await window.electronAPI.compareBranches(
            repoPath,
            currentBranch,
            branchName,
          );
            ...
            content: (
              <div style={{ marginTop: 16 }}>
                <div style={{ marginBottom: 16 }}>
                  <strong>Commits Ahead:</strong> {comparison.ahead}
                  <br />
    ...
    if (action === "checkout") {
      ...
      modal.confirm({
        ...
        content: (
          <div>
            <p>
              Merge branch <strong>{displayName}</strong> into current branch{" "}
                ...
                >
                  <Space direction="vertical">
                    <Radio value="no-ff">
                      ...
                      <div
                        style={{ fontSize: 12, color: "#666", marginLeft: 24 }}
                      >
                        Always create a merge commit, even if fast-forward is
                        possible
                      </div>
                    </Radio>
                    <Radio value="auto">
                      ...
                      <div
                        style={{ fontSize: 12, color: "#666", marginLeft: 24 }}
                      >
                        Fast-forward if possible, otherwise create merge commit
                      </div>
                    </Radio>
                    <Radio value="ff-only">
      ...
      if (parts.length === 1) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
          ...
          if (!currentNode.folders.has(folderName)) {
            currentNode.folders.set(folderName, {
              branches: [],
              folders: new Map(),
            });
          }
