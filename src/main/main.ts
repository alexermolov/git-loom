...
// Import from modular git services
import {
  // Commit operations
  getCommits,
  getUnpushedCommits,
...
function setupIpcHandlers() {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, forceFetch?: boolean) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, branch?: string, maxCount?: number) => {
      try {
  ...
  ipcMain.handle(
    "git:deleteBranch",
    async (_event, repoPath: string, branchName: string, force: boolean) => {
      try {
  ...
  ipcMain.handle(
    "git:getReflog",
    async (_event, repoPath: string, ref?: string, maxCount?: number) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, commitHash: string) => {
      try {
        await revertCommit(repoPath, commitHash);
      } catch (error) {
  ...
  // Abort revert
  ipcMain.handle("git:abortRevert", async (_event, repoPath: string) => {
    try {
      await abortRevert(repoPath);
    } catch (error) {
  ...
  // Continue revert
  ipcMain.handle("git:continueRevert", async (_event, repoPath: string) => {
    try {
      return await continueRevert(repoPath);
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, index: number, branchName: string) => {
      try {
  ...
  ipcMain.handle(
    "git:searchCommits",
    async (_event, repoPath: string, filter: any, limit?: number) => {
      try {
  ...
  ipcMain.handle(
    "git:searchCommitsMultiRepo",
    async (_event, repoPaths: string[], filter: any, limit?: number) => {
      try {
  ...
  ipcMain.handle(
    "git:fetchRemote",
    async (_event, repoPath: string, remoteName: string, prune?: boolean) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, tagName: string, commitHash?: string) => {
      try {
  ...
  ipcMain.handle(
    "git:deleteTag",
    async (_event, repoPath: string, tagName: string) => {
      try {
  ...
  ipcMain.handle(
    "git:deleteRemoteTag",
    async (_event, repoPath: string, remoteName: string, tagName: string) => {
      try {
